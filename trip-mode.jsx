...
const RecommendationsApp = () => {
  ...
  const [discoveryFeatures, setDiscoveryFeatures] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    Promise.all([
      fetch("http://localhost:3000/api/spots").then(res => res.json()),
      fetch("http://localhost:3000/api/discovery-features/pch_seg_22").then(res => res.json()),
      fetch("http://localhost:3000/api/discovery-features/carmel_entry").then(res => res.json())
    ]).then(([spots, df1, df2]) => {
      setMapSpots(spots || []);
      // Ensure we don't end up with undefined inside the array if features returned empty
      const validDf = [...(df1 || []), ...(df2 || [])].filter(Boolean);
      setDiscoveryFeatures(validDf);
      setLoading(false);
    }).catch(err => {
      console.error(err);
      setLoading(false);
  ...
  if (loading || mapSpots.length === 0) {
    return (
      <div style={{ height: 400, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", color: A, fontFamily: "'Anybody',sans-serif", fontSize: 13, background: "rgba(255,255,255,0.02)", borderRadius: 16, border: `1px solid ${BDR}` }}>
        <div style={{ width: 24, height: 24, border: `2px solid ${A}`, borderTopColor: "transparent", borderRadius: "50%", animation: "spin 1s linear infinite", marginBottom: 12 }} />
    ...
    <div>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        {discoveryFeatures.length === 0 ? (
          <div style={{ padding: 24, textAlign: 'center', color: '#666', background: "rgba(255,255,255,0.02)", borderRadius: 16, border: `1px solid ${BDR}`, fontSize: 12, fontFamily: "'Instrument Sans',sans-serif" }}>No dynamic suggestions generated for this location block.</div>
